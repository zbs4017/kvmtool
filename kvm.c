...
const char *kvm__get_dir(void) { return kvm_dir; }
...
int kvm__destroy_mem(struct kvm *kvm, u64 guest_phys, u64 size,
  ...
  list_for_each_entry(bank, &kvm->mem_banks,
                      list) if (bank->guest_phys_addr == guest_phys &&
                                bank->size == size &&
  ...
  if (bank->type == KVM_MEM_TYPE_RESERVED) {
    pr_err("Cannot delete reserved region [%llx-%llx]", guest_phys,
           guest_phys + size - 1);
    ret = -EINVAL;
    goto out;
  }
...
int kvm__register_mem(struct kvm *kvm, u64 guest_phys, u64 size,
  ...
  //如果这个内存区域不是reserved的，那么就需要注册到kvm中
...
int kvm__init(struct kvm *kvm) {
  ...
  if (kvm->cfg.firmware_filename) {
    if (!kvm__load_firmware(kvm, kvm->cfg.firmware_filename))
      die("unable to load firmware image %s: %s", kvm->cfg.firmware_filename,
          strerror(errno));
  } else {
    ret = kvm__arch_setup_firmware(kvm);//初始化firmware
    if (ret < 0)
...
void kvm__dump_mem(struct kvm *kvm, unsigned long addr, unsigned long size,
  ...
  for (n = 0; n < size; n += 8) {
    ...
    dprintf(debug_fd, " 0x%08lx: %02x %02x %02x %02x  %02x %02x %02x %02x\n",
            addr + n, p[n + 0], p[n + 1], p[n + 2], p[n + 3], p[n + 4],
            p[n + 5], p[n + 6], p[n + 7]);
  }
}
...
void kvm__continue(struct kvm *kvm) { mutex_unlock(&pause_lock); }
