...
static const char *const setup_usage[] = {"lkvm setup [name]", NULL};

static const struct option setup_options[] = {OPT_END()};
...
static int copy_file(const char *from, const char *to) {
  ...
  out_fd = open(to, O_RDWR | O_CREAT | O_TRUNC,
...
static const char *guestfs_dirs[] = {
    "/dev", "/etc", "/home", "/host",    "/proc", "/root",
    "/sys", "/tmp", "/var",  "/var/lib", "/virt", "/virt/home",
};

static const char *guestfs_symlinks[] = {
    "/bin", "/lib", "/lib64", "/sbin", "/usr", "/etc/ld.so.conf",
};
...
int kvm_setup_guest_init(const char *guestfs_name) {
  ...
  err = extract_file(guestfs_name, "virt/pre_init", pre_init_binary,
                     pre_init_binary_size);
  if (err)
    return err;
#endif
...
void kvm_setup_resolv(const char *guestfs_name) {
  ...
  snprintf(path, PATH_MAX, "%s%s/etc/resolv.conf", kvm__get_dir(),
           guestfs_name);
...
int kvm_cmd_setup(int argc, const char **argv, const char *prefix) {
  ...
  if (r == 0) {
    pr_info("A new rootfs '%s' has been created in '%s%s'.", instance_name,
            kvm__get_dir(), instance_name);
    pr_info("You can now start it by running the following command:");
    pr_info("%s run -d %s", KVM_BINARY_NAME, instance_name);
  } else {
    pr_err("Unable to create rootfs in %s%s: %s", kvm__get_dir(), instance_name,
           strerror(errno));
  }
