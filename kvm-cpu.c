...
static void kvm_cpu__handle_coalesced_mmio(struct kvm_cpu *cpu) {
  if (cpu->ring) {
    while (cpu->ring->first != cpu->ring->last) {
      ...
      kvm_cpu__emulate_mmio(cpu, m->phys_addr, m->data, m->len, 1);
...
int kvm_cpu__start(struct kvm_cpu *cpu) {
  ...
  while (cpu->is_running) {
    ...
    case KVM_EXIT_IO: {
      ...
      ret =
          kvm_cpu__emulate_io(cpu, cpu->kvm_run->io.port,
                              (u8 *)cpu->kvm_run + cpu->kvm_run->io.data_offset,
                              cpu->kvm_run->io.direction, cpu->kvm_run->io.size,
    ...
    case KVM_EXIT_MMIO: {
      ...
      ret = kvm_cpu__emulate_mmio(
          cpu, cpu->kvm_run->mmio.phys_addr, cpu->kvm_run->mmio.data,
          cpu->kvm_run->mmio.len, cpu->kvm_run->mmio.is_write);
...
int kvm_cpu__init(struct kvm *kvm) {
  ...
  max_cpus = kvm__max_cpus(kvm);//得到host的cpu数量
  recommended_cpus = kvm__recommended_cpus(kvm);//得到推荐创建的cpu数量
