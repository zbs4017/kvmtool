...
static asymbol *lookup(asymbol **symbols, int nr_symbols,
...
char *symbol_lookup(struct kvm *kvm, unsigned long addr, char *sym,
                    size_t size) {
  const char *filename;
  bfd_vma sym_offset;
  bfd_vma sym_start;
  ...
  snprintf(sym, size, "%s+%llx (%s:%i)", func, (long long)sym_offset, filename,
           line);
