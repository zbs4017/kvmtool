...
static const char *const run_usage[] = {"lkvm run [<options>] [<kernel image>]",
                                        NULL};
...
static int img_name_parser(const struct option *opt, const char *arg,
                           int unset) {
  char path[PATH_MAX];
  struct stat st;
...
void kvm_run_set_wrapper_sandbox(void) { kvm_run_wrapper = KVM_RUN_SANDBOX; }
...
static int loglevel_parser(const struct option *opt, const char *arg,
                           int unset) {
...
#define BUILD_OPTIONS(name, cfg, kvm)                                          \
  struct option name[] = {                                                     \
      ...
      OPT_CALLBACK('m', "mem", NULL, MEM_OPT_HELP_SHORT, MEM_OPT_HELP_LONG,    \
                   mem_parser, kvm),                                           \
      OPT_CALLBACK('d', "disk", kvm, "image or rootfs_dir",                    \
                   "Disk "                                                     \
                   " image or rootfs directory",                               \
                   img_name_parser, kvm),                                      \
      OPT_BOOLEAN('\0', "balloon", &(cfg)->balloon,                            \
                  "Enable virtio"                                              \
                  " balloon"),                                                 \
      ...
      OPT_BOOLEAN('\0', "nodefaults", &(cfg)->nodefaults,                      \
                  "Disable"                                                    \
      ...
      OPT_CALLBACK('\0', "9p", NULL, "dir_to_share,tag_name",                  \
                   ...
                   " guest",                                                   \
                   virtio_9p_rootdir_parser, kvm),                             \
      OPT_STRING('\0', "console", &(cfg)->console,                             \
                 "serial, virtio or"                                           \
                 " hv",                                                        \
                 "Console to use"),                                            \
      OPT_U64('\0', "vsock", &(cfg)->vsock_cid, "Guest virtio socket CID"),    \
      OPT_STRING('\0', "dev", &(cfg)->dev, "device_file", "KVM device file"),  \
      OPT_CALLBACK('\0', "tty", NULL, "tty id",                                \
                   "Remap guest TTY into a pty on the host", tty_parser,       \
                   NULL),                                                      \
      ...
      OPT_STRING('\0', "hugetlbfs", &(cfg)->hugetlbfs_path, "path",            \
                 "Hugetlbfs path"),                                            \
      OPT_CALLBACK_NOOPT('\0', "virtio-legacy", &(cfg)->virtio_transport, "",  \
      ...
      OPT_CALLBACK('\0', "virtio-transport", &(cfg)->virtio_transport,         \
                   VIRTIO_TRANS_OPT_HELP_SHORT, "Type of virtio transport",    \
                   virtio_transport_parser, NULL),                             \
      ...
      OPT_BOOLEAN('\0', "no-dhcp", &(cfg)->no_dhcp,                            \
                  "Disable kernel"                                             \
                  " DHCP in rootfs mode"),                                     \
                                                                               \
...
static const char *host_kernels[] = {"/boot/vmlinuz", "/boot/bzImage", NULL};

static const char *default_kernels[] = {
    "./bzImage", "arch/" BUILD_ARCH "/boot/bzImage",
    "../../arch/" BUILD_ARCH "/boot/bzImage", NULL};

static const char *default_vmlinux[] = {"vmlinux", "../../../vmlinux",
                                        "../../vmlinux", NULL};
...
static void kernel_usage_with_options(void) {
  ...
  pr_info("Please see '%s run --help' for more options.", KVM_BINARY_NAME);
}
...
static void kvm_write_sandbox_cmd_exactly(int fd, const char *arg) {
  ...
  while (*arg) {
    ...
    if (arg != single_quote) {
      if (write(fd, "'", 1) <= 0 || write(fd, arg, single_quote - arg) <= 0 ||
          write(fd, "'", 1) <= 0)
        die("Failed writing sandbox script");
    }
...
static void kvm_run_write_sandbox_cmd(struct kvm *kvm, const char **argv,
                                      int argc) {
...
static void kvm_run_set_real_cmdline(struct kvm *kvm) {
  ...
  if (kvm->cfg.using_rootfs) {
    strcat(real_cmdline,
           " rw rootflags=trans=virtio,version=9p2000.L,cache=loose "
           "rootfstype=9p");
...
static void kvm_run_validate_cfg(struct kvm *kvm) {
  ...
  if (kvm->cfg.ram_size) {
    ...
    if (available_ram && kvm->cfg.ram_size > available_ram) {
      pr_warning(
          "Guest memory size %lluMB exceeds host physical RAM size %lluMB",
...
static struct kvm *kvm_cmd_run_init(int argc, const char **argv) {
  ...
  while (argc != 0) {
    BUILD_OPTIONS(options, &kvm->cfg, kvm);
    argc =
        parse_options(argc, argv, options, run_usage,
  ...
  if (!kvm->cfg.nodefaults && !kvm->cfg.using_rootfs &&
      !kvm->cfg.disk_image[0].filename && !kvm->cfg.initrd_filename) {
    char tmp[PATH_MAX];
  ...
  if (kvm->cfg.kernel_filename) {
    pr_info("# %s run -k %s -m %Lu -c %d --name %s", KVM_BINARY_NAME,
            ...
            (unsigned long long)kvm->cfg.ram_size >> MB_SHIFT, kvm->cfg.nrcpus,
            kvm->cfg.guest_name);
...
static int kvm_cmd_run_work(struct kvm *kvm) {
  ...
  for (i = 0; i < kvm->nrcpus; i++) {
    if (pthread_create(&kvm->cpus[i]->thread, NULL, kvm_cpu_thread,
                       kvm->cpus[i]) != 0)
