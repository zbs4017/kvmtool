...
static struct mmio_mapping *mmio_search(struct rb_root *root, u64 addr,
                                        u64 len) {
  struct rb_int_node *node;
...
static struct mmio_mapping *mmio_get(struct rb_root *root, u64 phys_addr,
                                     u32 len) {
...
static void mmio_deregister(struct kvm *kvm, struct rb_root *root,
                            struct mmio_mapping *mmio) {
...
static void mmio_put(struct kvm *kvm, struct rb_root *root,
                     struct mmio_mapping *mmio) {
  mutex_lock(&mmio_lock);
  mmio->refcount--;
  ...
  if (!mmio) {
    if (vcpu->kvm->cfg.mmio_debug)
      fprintf(stderr, "MMIO warning: Ignoring MMIO %s at %016llx (length %u)\n",
              to_direction(is_write), (unsigned long long)phys_addr, len);
    goto out;
  }
