...
static gboolean kvm_gtk_configure_event(GtkWidget *widget,
  ...
  surface = cairo_image_surface_create_for_data(
      (void *)fb->mem, CAIRO_FORMAT_RGB24, fb->width, fb->height, stride);
...
static gboolean kvm_gtk_key_press(GtkWidget *widget, GdkEventKey *event,
                                  gpointer user_data) {
...
static void *kvm_gtk_thread(void *p) {
  ...
  g_signal_connect(da, "configure-event", G_CALLBACK(kvm_gtk_configure_event),
                   fb);
  g_signal_connect(G_OBJECT(window), "key_press_event",
                   G_CALLBACK(kvm_gtk_key_press), NULL);
