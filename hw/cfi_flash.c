...
#define FLASH_BLOCK_SIZE_PER_CHIP (FLASH_BLOCK_SIZE / CFI_NR_FLASH_CHIPS)
...
#define CFI_STATUS_LOCK_ERROR                                                  \
  CFI_STATUS_PROGRAM_LOCK_BIT | CFI_STATUS_PROTECT_BIT
#define CFI_STATUS_ERASE_ERROR                                                 \
  CFI_STATUS_ERASE_CLEAR_LOCK_BIT | CFI_STATUS_PROGRAM_LOCK_BIT
...
static const u8 cfi_query_table[] = {
    ...
    'Y', /* ID string */
    0x01,
    0x00, /* primary command set: Intel/Sharp extended */
    0x31,
    0x00, /* address of primary extended query table */
    0x00,
    0x00, /* alternative command set: unused */
    0x00,
    0x00,          /* address of alternative extended query table*/
    ...
    0x05,
    0x00, /* interface description: 32 and 16 bits */
    PROGRAM_BUFF_SIZE_BITS_PER_CHIP,
    0x00,
    ...
    0x00,
    0x00,
    0x00, /* number and size of erase blocks, generated */
    ...
    0x00,
    0x00,
    0x00, /* optional features: instant lock & pm-read */
    0x00, /* no functions after suspend */
    0x01,
    0x00, /* only lock bit supported */
    ...
    0x00,
    0x00,
    0x00, /* protection field 1 description */
};
...
static void word_program(struct cfi_flash_device *sfdev, u64 faddr, void *data,
                         int len) {
...
static bool buffer_write(struct cfi_flash_device *sfdev, u64 faddr,
                         void *buffer, int len) {
...
static void buffer_confirm(struct cfi_flash_device *sfdev) {
  ...
  memcpy(sfdev->flash_memory + sfdev->block_address, sfdev->program_buffer,
         sfdev->buff_written);
}
...
static void cfi_flash_read(struct cfi_flash_device *sfdev, u64 faddr, u8 *data,
                           u32 len) {
  u16 cfi_value = 0;
  ...
  case 2:
    ...
    break;
  default:
    pr_debug("CFI flash: illegal access length %d for read mode %d", len,
             sfdev->read_mode);
    break;
  }
}
...
static void cfi_flash_write(struct cfi_flash_device *sfdev, u16 command,
  ...
  case BUFFER_WRITE:
    ...
    if ((command & 0xff) == CFI_CMD_CONFIRM) {
      ...
      pr_debug(
          "CFI flash: BUFFER_WRITE: expected CONFIRM(0xd0), got 0x%x @ 0x%llx",
          command, faddr);
...
static int map_flash_memory(struct kvm *kvm, struct cfi_flash_device *sfdev) {
  ...
  ret =
      kvm__register_mem(kvm, sfdev->base_addr, sfdev->size, sfdev->flash_memory,
...
static int unmap_flash_memory(struct kvm *kvm, struct cfi_flash_device *sfdev) {
  ...
  ret =
      kvm__destroy_mem(kvm, sfdev->base_addr, sfdev->size, sfdev->flash_memory);
...
static void cfi_flash_mmio(struct kvm_cpu *vcpu, u64 addr, u8 *data, u32 len,
                           u8 is_write, void *context) {
  ...
  if (len > 4) {
    pr_info("CFI flash: MMIO %d-bit write access not supported", len * 8);
    return;
  }
...
static void generate_cfi_flash_fdt_node(
    void *fdt, struct device_header *dev_hdr,
    void (*generate_irq_prop)(void *fdt, u8 irq, enum irq_type)) {
...
static struct cfi_flash_device *create_flash_device_file(struct kvm *kvm,
  ...
  sfdev->flash_memory =
      mmap(NULL, sfdev->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  ...
  ret = kvm__register_mmio(kvm, sfdev->base_addr, sfdev->size, false,
                           cfi_flash_mmio, sfdev);
  if (ret) {
