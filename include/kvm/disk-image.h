...
struct disk_image *disk_image__new(int fd, u64 size,
                                   struct disk_image_operations *ops, int mmap);
...
struct disk_image *blkdev__probe(const char *filename, int flags,
                                 struct stat *st);
...
static inline int raw_image__wait(struct disk_image *disk) { return 0; }
