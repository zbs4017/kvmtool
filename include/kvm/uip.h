...
#define UIP_DHCP_MAX_PAYLOAD_LEN                                               \
  (UIP_BOOTP_MAX_PAYLOAD_LEN - UIP_BOOTP_VENDOR_SPECIFIC_LEN +                 \
   UIP_DHCP_VENDOR_SPECIFIC_LEN)
#define UIP_DHCP_OPTION_LEN                                                    \
  (UIP_DHCP_VENDOR_SPECIFIC_LEN - UIP_DHCP_MAGIC_COOKIE_LEN)
...
static inline u16 uip_ip_len(struct uip_ip *ip) { return htons(ip->len); }

static inline u16 uip_udp_hdrlen(struct uip_udp *udp) { return 8; }

static inline u16 uip_udp_len(struct uip_udp *udp) { return ntohs(udp->len); }
...
static inline u32 uip_tcp_isn(struct uip_tcp *tcp) { return ntohl(tcp->seq); }
...
static inline u16 uip_eth_hdrlen(struct uip_eth *eth) { return sizeof(*eth); }
...
int uip_udp_make_pkg(struct uip_info *info, struct uip_udp_socket *sk,
                     struct uip_buf *buf, u8 *payload, int payload_len);
