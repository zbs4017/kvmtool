...
enum kvm_mem_type {
  ...
  KVM_MEM_TYPE_ALL = KVM_MEM_TYPE_RAM | KVM_MEM_TYPE_DEVICE |
                     KVM_MEM_TYPE_RESERVED | KVM_MEM_TYPE_READONLY
};
...
bool kvm__emulate_io(struct kvm_cpu *vcpu, u16 port, void *data, int direction,
                     int size, u32 count);
...
kvm__register_mmio(struct kvm *kvm, u64 phys_addr, u64 phys_addr_len,
                   bool coalesce, mmio_handler_fn mmio_fn, void *ptr) {
  return kvm__register_iotrap(kvm, phys_addr, phys_addr_len, mmio_fn, ptr,
                              DEVICE_BUS_MMIO |
                                  (coalesce ? IOTRAP_COALESCE : 0));
}
static inline int __must_check kvm__register_pio(struct kvm *kvm, u16 port,
                                                 u16 len,
...
static inline bool kvm__arch_has_cfg_ram_address(void) { return true; }
#else
static inline bool kvm__arch_has_cfg_ram_address(void) { return false; }
#endif
...
int kvm__for_each_mem_bank(struct kvm *kvm, enum kvm_mem_type type,
                           int (*fun)(struct kvm *kvm,
                                      struct kvm_mem_bank *bank, void *data),
                           void *data);
