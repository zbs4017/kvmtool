...
#define pci_dev_err(pci_hdr, fmt, ...)                                         \
  pr_err("[%04x:%04x] " fmt, (pci_hdr)->vendor_id, (pci_hdr)->device_id,       \
         ##__VA_ARGS__)
#define pci_dev_warn(pci_hdr, fmt, ...)                                        \
  pr_warning("[%04x:%04x] " fmt, (pci_hdr)->vendor_id, (pci_hdr)->device_id,   \
             ##__VA_ARGS__)
#define pci_dev_info(pci_hdr, fmt, ...)                                        \
  pr_info("[%04x:%04x] " fmt, (pci_hdr)->vendor_id, (pci_hdr)->device_id,      \
          ##__VA_ARGS__)
#define pci_dev_dbg(pci_hdr, fmt, ...)                                         \
  pr_debug("[%04x:%04x] " fmt, (pci_hdr)->vendor_id, (pci_hdr)->device_id,     \
           ##__VA_ARGS__)
#define pci_dev_die(pci_hdr, fmt, ...)                                         \
  die("[%04x:%04x] " fmt, (pci_hdr)->vendor_id, (pci_hdr)->device_id,          \
      ##__VA_ARGS__)
...
struct msix_cap {
  ...
  u8 next;
  u16 ctrl;         // 控制寄存器
  u32 table_offset; // MSI-X表的BAR索引和偏移
  u32 pba_offset;   // PBA(Pending Bit Array)的BAR索引和偏移
};
...
struct pci_config_operations {
  void (*write)(struct kvm *kvm, struct pci_device_header *pci_hdr, u16 offset,
                void *data, int sz);
  void (*read)(struct kvm *kvm, struct pci_device_header *pci_hdr, u16 offset,
               void *data, int sz);
};
...
struct pci_device_header {
  ...
  union {
    struct {
      ...
      u16 subsys_id;
      u32 exp_rom_bar; // 拓展rom基址
      u8 capabilities; // 能力指针
      u8 reserved1[3]; // 保留
      u32 reserved2;   // 保留
      u8 irq_line;     // 中断线
      u8 irq_pin;      // 中断引脚
    ...
    u8 __pad[PCI_DEV_CFG_SIZE]; // 整个union是256字节
  };
...
static inline bool
pci__memory_space_enabled(struct pci_device_header *pci_hdr) {
...
static inline bool pci__bar_is_io(struct pci_device_header *pci_hdr,
                                  int bar_num) {
  return __pci__bar_is_io(pci_hdr->bar[bar_num]);
}

static inline bool pci__bar_is_memory(struct pci_device_header *pci_hdr,
                                      int bar_num) {
...
static inline u32 pci__bar_address(struct pci_device_header *pci_hdr,
                                   int bar_num) {
  return __pci__bar_address(pci_hdr->bar[bar_num]);
}

static inline u32 pci__bar_size(struct pci_device_header *pci_hdr,
                                int bar_num) {
