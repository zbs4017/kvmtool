...
void kvm__arch_init(struct kvm *kvm) {
  ...
  kvm->ram_start =
      mmap_anon_or_hugetlbfs(kvm, kvm->cfg.hugetlbfs_path, kvm->ram_size);
...
static bool load_flat_binary(struct kvm *kvm, int fd_kernel) {
  ...
  kernel_size = read_file(fd_kernel, p, kvm->cfg.ram_size - KERNEL_LOAD_ADDR);
  if (kernel_size == -1) {
    if (errno == ENOMEM)
  ...
  pr_info("Loaded kernel to 0x%x (%zd bytes)", KERNEL_LOAD_ADDR, kernel_size);
