...
static void (*msgs[KVM_IPC_MAX_MSGS])(struct kvm *kvm, int fd, u32 type,
                                      u32 len, u8 *msg);
...
static int kvm__create_socket(struct kvm *kvm) {
  ...
  snprintf(full_name, sizeof(full_name), "%s/%s%s", kvm__get_dir(),
           kvm->cfg.guest_name, KVM_SOCK_SUFFIX);
...
void kvm__remove_socket(const char *name) {
  ...
  snprintf(full_name, sizeof(full_name), "%s/%s%s", kvm__get_dir(), name,
           KVM_SOCK_SUFFIX);
  unlink(full_name);
}
...
int kvm__get_sock_by_instance(const char *name) {
  ...
  snprintf(sock_file, sizeof(sock_file), "%s/%s%s", kvm__get_dir(), name,
           KVM_SOCK_SUFFIX);
  s = socket(AF_UNIX, SOCK_STREAM, 0);
...
int kvm_ipc__register_handler(u32 type,
                              void (*cb)(struct kvm *kvm, int fd, u32 type,
                                         u32 len, u8 *msg)) {
...
static int kvm_ipc__handle(struct kvm *kvm, int fd, u32 type, u32 len,
                           u8 *data) {
...
static void handle_vmstate(struct kvm *kvm, int fd, u32 type, u32 len,
                           u8 *msg) {
  int r = 0;
