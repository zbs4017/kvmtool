...
static void mptable_add_irq_src(struct mpc_intsrc *mpc_intsrc, u16 srcbusid,
                                u16 srcbusirq, u16 dstapic, u16 dstirq) {
  *mpc_intsrc = (struct mpc_intsrc){.type = MP_INTSRC,
                                    .irqtype = mp_INT,
...
int mptable__init(struct kvm *kvm) {
  ...
  if (ncpus > MPTABLE_MAX_CPUS) {
    pr_warning("Too many cpus: %d limited to %d", ncpus, MPTABLE_MAX_CPUS);
  ...
  while (dev_hdr) {
    ...
    mptable_add_irq_src(mpc_intsrc, pcibusid, srcbusirq, ioapicid,
                        pci_hdr->irq_line);
   ...
   */
  real_mpf_intel =
      ALIGN((unsigned long)last_addr - (unsigned long)mpc_table, 16);
  ...
  mpf_intel->checksum =
      -mpf_checksum((unsigned char *)mpf_intel, sizeof(*mpf_intel));
  ...
  mpc_table->checksum =
      -mpf_checksum((unsigned char *)mpc_table, mpc_table->length);
   ...
   */
  size =
      (unsigned long)mpf_intel + sizeof(*mpf_intel) - (unsigned long)mpc_table;
