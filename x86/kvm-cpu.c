...
static struct kvm_msrs *kvm_msrs__new(size_t nmsrs) {
  struct kvm_msrs *vcpu =
      calloc(1, sizeof(*vcpu) + (sizeof(struct kvm_msr_entry) * nmsrs));
...
bool kvm_cpu__handle_exit(struct kvm_cpu *vcpu) { return false; }

static void print_dtable(const char *name, struct kvm_dtable *dtable) {
  dprintf(debug_fd, " %s                 %016llx  %08hx\n", name,
          (u64)dtable->base, (u16)dtable->limit);
}

static void print_segment(const char *name, struct kvm_segment *seg) {
  dprintf(
      debug_fd,
      " %s       %04hx      %016llx  %08x  %02hhx    %x %x   %x  %x %x %x %x\n",
      name, (u16)seg->selector, (u64)seg->base, (u32)seg->limit, (u8)seg->type,
...
void kvm_cpu__show_registers(struct kvm_cpu *vcpu) {
  ...
  dprintf(debug_fd, "\n Segment registers:\n");
  dprintf(debug_fd, " ------------------\n");
  dprintf(debug_fd, " register  selector  base              limit     type  p "
                    "dpl db s l g avl\n");
...
void kvm_cpu__show_code(struct kvm_cpu *vcpu) {
  ...
  ip = guest_flat_to_host(vcpu->kvm,
...
void kvm_cpu__show_page_tables(struct kvm_cpu *vcpu) {
  ...
  if (*pte2 & (1 << 7))
    dprintf(debug_fd,
            " pte4: %016llx   pte3: %016llx"
            "   pte2: %016llx\n",
            *pte4, *pte3, *pte2);
  else
    dprintf(debug_fd,
            " pte4: %016llx  pte3: %016llx   pte2: %016"
            "llx   pte1: %016llx\n",
            *pte4, *pte3, *pte2, *pte1);
}
