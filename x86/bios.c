...
#define DEFINE_BIOS_IRQ_HANDLER(_irq, _handler)                                \
  {                                                                            \
    .irq = _irq, .address = BIOS_IRQ_PA_ADDR(_handler),                        \
    .handler = BIOS_IRQ_FUNC(_handler), .size = BIOS_IRQ_SIZE(_handler),       \
  }
...
void setup_bios(struct kvm *kvm) {
   ...
   */
  // 在这里清空一些区域，bios占用了部分的地址空间，现在我们要虚拟化bios，这些地方之前的数据需要清空
