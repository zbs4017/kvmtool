...
static int readnum(const struct option *opt, int flags, const char *str,
                   char **end) {
  switch (opt->type) {
  case OPTION_INTEGER:
...
static int parse_long_opt(struct parse_opt_ctx_t *p, const char *arg,
  ...
  if (ambiguous_option) {
    pr_err("Ambiguous option: %s (could be --%s%s or --%s%s)", arg,
           (ambiguous_flags & OPT_UNSET) ? "no-" : "",
           ambiguous_option->long_name, (abbrev_flags & OPT_UNSET) ? "no-" : "",
