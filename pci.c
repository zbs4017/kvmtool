...
static bool pci_bar_is_implemented(struct pci_device_header *pci_hdr,
                                   int bar_num) {
...
static bool pci_device_exists(u8 bus_number, u8 device_number,
                              u8 function_number) {
...
static int pci_activate_bar(struct kvm *kvm, struct pci_device_header *pci_hdr,
  ...
  if (r < 0) {
    pci_dev_warn(pci_hdr, "Error activating emulation for BAR %d", bar_num);
    goto out;
  }
...
static int pci_deactivate_bar(struct kvm *kvm,
  ...
  if (r < 0) {
    pci_dev_warn(pci_hdr, "Error deactivating emulation for BAR %d", bar_num);
    goto out;
  }
...
static int pci_toggle_bar_regions(bool activate, struct kvm *kvm, u32 start,
                                  u32 size) {
  ...
  while (dev_hdr) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      if (tmp_start + tmp_size <= start || tmp_start >= start + size)
        continue;
...
static inline int pci_activate_bar_regions(struct kvm *kvm, u32 start,
                                           u32 size) {
  return pci_toggle_bar_regions(true, kvm, start, size);
}

static inline int pci_deactivate_bar_regions(struct kvm *kvm, u32 start,
                                             u32 size) {
...
static void pci_config_bar_wr(struct kvm *kvm,
  ...
  if (pci__bar_is_io(pci_hdr, bar_num) && !pci__io_space_enabled(pci_hdr)) {
    pci_hdr->bar[bar_num] = value;
    return;
  }
...
static const u8 pci_config_writable[PCI_STD_HEADER_SIZEOF] = {
    [PCI_COMMAND] = PCI_COMMAND_IO | PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER |
                    PCI_COMMAND_PARITY,
    [PCI_COMMAND + 1] = (PCI_COMMAND_SERR | PCI_COMMAND_INTX_DISABLE) >> 8,
    [PCI_INTERRUPT_LINE] = 0xff,
    [PCI_BASE_ADDRESS_0... PCI_BASE_ADDRESS_5 + 3] = 0xff,
    [PCI_CACHE_LINE_SIZE] = 0xff,
};

void pci__config_wr(struct kvm *kvm, union pci_config_address addr, void *data,
                    int size) {
  void *base;
  u8 bar;
  u16 offset;
...
void pci__config_rd(struct kvm *kvm, union pci_config_address addr, void *data,
                    int size) {
  u16 offset;
  struct pci_device_header *pci_hdr;
  u8 dev_num = addr.device_number;
...
int pci__register_bar_regions(struct kvm *kvm,
                              struct pci_device_header *pci_hdr,
  ...
  for (i = 0; i < 6; i++) {
    // 如果这个bar没有实现，就跳过，没实现一般值得是这个设备的中的这个bar没有使用
    if (!pci_bar_is_implemented(pci_hdr, i))
      continue;

    assert(!pci_bar_is_active(pci_hdr, i));

    // 如果这个bar是io类型的，并且io空间是使能的，就激活这个bar
    if (pci__bar_is_io(pci_hdr, i) && pci__io_space_enabled(pci_hdr)) {
      r = pci_activate_bar(kvm, pci_hdr, i);
      if (r < 0)
        return r;
    }
    // 如果这个bar是memory类型的，并且memory空间是使能的，就激活这个bar
    if (pci__bar_is_memory(pci_hdr, i) && pci__memory_space_enabled(pci_hdr)) {
      r = pci_activate_bar(kvm, pci_hdr, i);
      if (r < 0)
        return r;
    }
...
int pci__init(struct kvm *kvm) {
  ...
  if (r < 0)
    return r;
  r = kvm__register_pio(kvm, PCI_CONFIG_ADDRESS, 4, pci_config_address_mmio,
                        NULL);
  if (r < 0)
