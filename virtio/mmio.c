...
#else
static void
generate_virtio_mmio_fdt_node(void *fdt, struct device_header *dev_hdr,
...
int virtio_mmio_init(struct kvm *kvm, void *dev, struct virtio_device *vdev,
  ...
  r = kvm__register_mmio(
      kvm, vmmio->addr, VIRTIO_MMIO_IO_SIZE, false,
      legacy ? virtio_mmio_legacy_callback : virtio_mmio_modern_callback, vdev);
  if (r < 0)
    return r;
   ...
   */
  pr_debug("virtio-mmio.devices=0x%x@0x%x:%d", VIRTIO_MMIO_IO_SIZE, vmmio->addr,
           vmmio->irq);
