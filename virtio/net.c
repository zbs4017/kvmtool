...
static int virtio_net_hdr_len(struct net_dev *ndev) {
  if (has_virtio_feature(ndev, VIRTIO_NET_F_MRG_RXBUF) || !ndev->vdev.legacy)
...
static void *virtio_net_rx_thread(void *p) {
  ...
  while (1) {
    ...
    while (virt_queue__available(vq)) {
      unsigned char
          buffer[MAX_PACKET_SIZE + sizeof(struct virtio_net_hdr_mrg_rxbuf)];
...
virtio_net_handle_mq(struct kvm *kvm, struct net_dev *ndev,
...
static u64 get_host_features(struct kvm *kvm, void *dev) {
  ...
  features = 1UL << VIRTIO_NET_F_MAC | 1UL << VIRTIO_NET_F_CSUM |
             1UL << VIRTIO_NET_F_HOST_TSO4 | 1UL << VIRTIO_NET_F_HOST_TSO6 |
             1UL << VIRTIO_NET_F_GUEST_TSO4 | 1UL << VIRTIO_NET_F_GUEST_TSO6 |
             1UL << VIRTIO_RING_F_EVENT_IDX |
             1UL << VIRTIO_RING_F_INDIRECT_DESC | 1UL << VIRTIO_NET_F_CTRL_VQ |
...
static void virtio_net_start(struct net_dev *ndev) {
  ...
  if (ndev->mode == NET_MODE_TAP) {
    ...
    if (ndev->vhost_fd && virtio_vhost_set_features(ndev->vhost_fd, features))
...
static void virtio_net_update_endian(struct net_dev *ndev) {
  ...
  conf->status =
      virtio_host_to_guest_u16(ndev->vdev.endian, VIRTIO_NET_S_LINK_UP);
  conf->max_virtqueue_pairs =
      virtio_host_to_guest_u16(ndev->vdev.endian, ndev->queue_pairs);

  /* Let TAP know about vnet header endianness */
  if (ndev->mode == NET_MODE_TAP && ndev->vdev.endian != VIRTIO_ENDIAN_HOST) {
...
static inline void str_to_mac(const char *str, char *mac) {
  sscanf(str, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", mac, mac + 1, mac + 2, mac + 3,
         mac + 4, mac + 5);
}
static int set_net_param(struct kvm *kvm, struct virtio_net_params *p,
  ...
  if (strcmp(param, "guest_mac") == 0) {
    ...
    if (!strncmp(val, "user", 4)) {
      ...
      die("Unknown network mode %s, please use user, tap or none",
          kvm->cfg.network);
...
static int virtio_net__init_one(struct virtio_net_params *params) {
  ...
  if (ndev->mode == NET_MODE_TAP) {
    ...
    ndev->info.buf_nr = 20, ndev->ops = &uip_ops;
  ...
  if (params->trans) {
    if (strcmp(params->trans, "mmio") == 0)
      ...
      pr_warning("virtio-net: Unknown transport method : %s, "
                 "falling back to %s.",
                 params->trans, virtio_trans_name(trans));
  }
...
int virtio_net__exit(struct kvm *kvm) {
  ...
  list_for_each_safe(ptr, n, &ndevs) {
    ...
    if (ndev->mode == NET_MODE_TAP && strcmp(params->downscript, "none"))
