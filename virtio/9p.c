...
static u16 virtio_p9_update_iov_cnt(struct iovec iov[], u32 count,
                                    int iov_cnt) {
  int i;
  u32 total = 0;
...
static void virtio_p9_error_reply(struct p9_dev *p9dev, struct p9_pdu *pdu,
                                  int err, u32 *outlen) {
  u16 tag;
...
static void virtio_p9_version(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  u32 msize;
  char *version;
...
static void virtio_p9_clunk(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  u32 fid;
...
static int stat_rel(struct p9_dev *p9dev, const char *path, struct stat *st) {
  ...
  if (get_full_path_helper(full_path, sizeof(full_path), p9dev->root_dir,
                           path) != 0)
    return -1;
...
static void virtio_p9_open(struct p9_dev *p9dev, struct p9_pdu *pdu,
                           u32 *outlen) {
  u32 fid, flags;
  struct stat st;
  struct p9_qid qid;
  ...
  if (is_dir(new_fid)) {
    ...
    new_fid->fd = open(new_fid->abs_path, virtio_p9_openflags(flags));
    if (new_fid->fd < 0)
      goto err_out;
  } else {
    goto err_out;
...
static void virtio_p9_create(struct p9_dev *p9dev, struct p9_pdu *pdu,
                             u32 *outlen) {
  int fd, ret;
  char *name;
  size_t size;
  ...
  virtio_p9_pdu_readf(pdu, "dsddd", &dfid_val, &name, &flags, &mode, &gid);
...
static void virtio_p9_mkdir(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  int ret;
  char *name;
  struct stat st;
  ...
  virtio_p9_pdu_readf(pdu, "dsdd", &dfid_val, &name, &mode, &gid);
  dfid = get_fid(p9dev, dfid_val);
...
static void virtio_p9_walk(struct p9_dev *p9dev, struct p9_pdu *pdu,
                           u32 *outlen) {
  u8 i;
  u16 nwqid;
  u16 nwname;
...
static void virtio_p9_attach(struct p9_dev *p9dev, struct p9_pdu *pdu,
                             u32 *outlen) {
  char *uname;
  char *aname;
  struct stat st;
  ...
  virtio_p9_pdu_readf(pdu, "ddssd", &fid_val, &afid, &uname, &aname, &uid);
...
static void virtio_p9_fill_stat(struct p9_dev *p9dev, struct stat *st,
...
static void virtio_p9_read(struct p9_dev *p9dev, struct p9_pdu *pdu,
                           u32 *outlen) {
  u64 offset;
  u32 fid_val;
  u16 iov_cnt;
  ...
  pdu->in_iov_cnt =
      virtio_p9_update_iov_cnt(pdu->in_iov, count, pdu->in_iov_cnt);
  rcount = preadv(fid->fd, pdu->in_iov, pdu->in_iov_cnt, offset);
  if (rcount > count)
    rcount = count;
  /*
...
static void virtio_p9_readdir(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  u32 fid_val;
  u32 count, rcount;
  struct stat st;
  ...
  while (dent) {
    ...
    virtio_p9_pdu_writef(pdu, "Qqbs", &qid, dent->d_off, dent->d_type,
                         dent->d_name);
...
static void virtio_p9_getattr(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  u32 fid_val;
  struct stat st;
  u64 request_mask;
...
static void virtio_p9_setattr(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  int ret = 0;
  u32 fid_val;
  struct p9_fid *fid;
  ...
  if ((p9attr.valid & (ATTR_UID | ATTR_GID)) ||
    ...
    ret = lchown(fid->abs_path, __kuid_val(p9attr.uid), __kgid_val(p9attr.gid));
    if (ret < 0)
      goto err_out;
  }
...
static void virtio_p9_write(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  ...
  pdu->out_iov_cnt =
      virtio_p9_update_iov_cnt(pdu->out_iov, count, pdu->out_iov_cnt);
...
static void virtio_p9_remove(struct p9_dev *p9dev, struct p9_pdu *pdu,
                             u32 *outlen) {
  int ret;
  u32 fid_val;
  struct p9_fid *fid;
...
static void virtio_p9_rename(struct p9_dev *p9dev, struct p9_pdu *pdu,
                             u32 *outlen) {
  int ret;
...
static void virtio_p9_readlink(struct p9_dev *p9dev, struct p9_pdu *pdu,
                               u32 *outlen) {
  int ret;
  u32 fid_val;
  struct p9_fid *fid;
...
static void virtio_p9_statfs(struct p9_dev *p9dev, struct p9_pdu *pdu,
                             u32 *outlen) {
  int ret;
  u64 fsid;
  u32 fid_val;
  ...
  virtio_p9_pdu_writef(pdu, "ddqqqqqqd", stat_buf.f_type, stat_buf.f_bsize,
                       stat_buf.f_blocks, stat_buf.f_bfree, stat_buf.f_bavail,
                       stat_buf.f_files, stat_buf.f_ffree, fsid,
                       stat_buf.f_namelen);
  *outlen = pdu->write_offset;
  virtio_p9_set_reply_header(pdu, *outlen);
  return;
...
static void virtio_p9_mknod(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  int ret;
  char *name;
  struct stat st;
  ...
  virtio_p9_pdu_readf(pdu, "dsdddd", &fid_val, &name, &mode, &major, &minor,
                      &gid);
...
static void virtio_p9_fsync(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  int ret, fd;
  struct p9_fid *fid;
...
static void virtio_p9_symlink(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  int ret;
  struct stat st;
  u32 fid_val, gid;
...
static void virtio_p9_link(struct p9_dev *p9dev, struct p9_pdu *pdu,
                           u32 *outlen) {
  int ret;
  char *name;
...
static void virtio_p9_lock(struct p9_dev *p9dev, struct p9_pdu *pdu,
                           u32 *outlen) {
  u8 ret;
  u32 fid_val;
  struct p9_flock flock;

  virtio_p9_pdu_readf(pdu, "dbdqqds", &fid_val, &flock.type, &flock.flags,
...
static void virtio_p9_getlock(struct p9_dev *p9dev, struct p9_pdu *pdu,
                              u32 *outlen) {
  u32 fid_val;
  struct p9_getlock glock;
  virtio_p9_pdu_readf(pdu, "dbqqds", &fid_val, &glock.type, &glock.start,
                      &glock.length, &glock.proc_id, &glock.client_id);

  /* Just return success */
  glock.type = F_UNLCK;
  virtio_p9_pdu_writef(pdu, "bqqds", glock.type, glock.start, glock.length,
...
static int virtio_p9_fix_path(struct p9_fid *fid, char *old_name,
                              char *new_name) {
  int ret;
  ...
  if (rp_sz == strlen(fid->path)) {
    ...
    ret = snprintf(tmp_name, sizeof(tmp_name), "%s%s", new_name,
                   fid->path + rp_sz);
...
static void virtio_p9_renameat(struct p9_dev *p9dev, struct p9_pdu *pdu,
                               u32 *outlen) {
  int ret;
  ...
  virtio_p9_pdu_readf(pdu, "dsds", &old_dfid_val, &old_name, &new_dfid_val,
                      &new_name);

  old_dfid = get_fid(p9dev, old_dfid_val);
  new_dfid = get_fid(p9dev, new_dfid_val);

  if (get_full_path(old_full_path, sizeof(old_full_path), old_dfid, old_name) !=
      0)
    goto err_out;

  if (get_full_path(new_full_path, sizeof(new_full_path), new_dfid, new_name) !=
      0)
    goto err_out;
...
static void virtio_p9_unlinkat(struct p9_dev *p9dev, struct p9_pdu *pdu,
                               u32 *outlen) {
  int ret;
  char *name;
  u32 fid_val, flags;
...
static void virtio_p9_flush(struct p9_dev *p9dev, struct p9_pdu *pdu,
                            u32 *outlen) {
  u16 tag, oldtag;
...
static void virtio_p9_eopnotsupp(struct p9_dev *p9dev, struct p9_pdu *pdu,
                                 u32 *outlen) {
  return virtio_p9_error_reply(p9dev, pdu, EOPNOTSUPP, outlen);
}

typedef void p9_handler(struct p9_dev *p9dev, struct p9_pdu *pdu, u32 *outlen);
...
static struct p9_pdu *virtio_p9_pdu_init(struct kvm *kvm,
  ...
  pdu->write_offset = VIRTIO_9P_HDR_LEN;
  pdu->queue_head = virt_queue__get_inout_iov(
      kvm, vq, pdu->in_iov, pdu->out_iov, &pdu->in_iov_cnt, &pdu->out_iov_cnt);
  return pdu;
}
...
static void notify_status(struct kvm *kvm, void *dev, u32 status) {
  ...
  if (status & VIRTIO__STATUS_CONFIG)
    p9dev->config->tag_len =
        virtio_host_to_guest_u16(p9dev->vdev.endian, p9dev->tag_len);
...
int virtio_9p_rootdir_parser(const struct option *opt, const char *arg,
                             int unset) {
  char *tag_name;
  char tmp[PATH_MAX];
...
int virtio_9p_img_name_parser(const struct option *opt, const char *arg,
                              int unset) {
  char path[PATH_MAX];
  struct stat st;
  struct kvm *kvm = opt->ptr;
...
int virtio_9p__register(struct kvm *kvm, const char *root,
                        const char *tag_name) {
  struct p9_dev *p9dev;
  size_t tag_length;
  size_t config_size;
