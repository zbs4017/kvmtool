...
typedef bool (*access_handler_t)(struct virtio_device *, unsigned long, void *,
                                 int);
...
static bool virtio_pci__common_write(struct virtio_device *vdev,
  ...
  case VIRTIO_PCI_COMMON_Q_SIZE:
    vdev->ops->set_size_vq(vpci->kvm, vpci->dev, vpci->queue_selector,
                           ioport__read16(data));
    break;
  case VIRTIO_PCI_COMMON_Q_MSIX:
    ...
    if (vdev->ops->notify_vq_gsi)
      vdev->ops->notify_vq_gsi(vpci->kvm, vpci->dev, vpci->queue_selector, gsi);
    break;
...
static bool virtio_pci__config_write(struct virtio_device *vdev,
  ...
  return virtio_access_config(vpci->kvm, vdev, vpci->dev,
                              offset - VPCI_CFG_DEV_START, data, size, true);
}
...
static bool virtio_pci__config_read(struct virtio_device *vdev,
  ...
  return virtio_access_config(vpci->kvm, vdev, vpci->dev,
                              offset - VPCI_CFG_DEV_START, data, size, false);
}
