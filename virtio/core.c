...
virt_queue__set_used_elem_no_update(struct virt_queue *queue, u32 head, u32 len,
                                    u16 offset) {
...
u16 virt_queue__get_head_iov(struct virt_queue *vq, struct iovec iov[],
  ...
  *out = *in = 0;
  ...
  if (virt_desc__test_flag(vq, &desc[idx], VRING_DESC_F_INDIRECT)) {
    max = virtio_guest_to_host_u32(vq->endian, desc[idx].len) /
          sizeof(struct vring_desc);
    desc = guest_flat_to_host(
        kvm, virtio_guest_to_host_u64(vq->endian, desc[idx].addr));
    idx = 0;
  }
  ...
  do {
    ...
    iov[*out + *in].iov_len =
        virtio_guest_to_host_u32(vq->endian, desc[idx].len);
    iov[*out + *in].iov_base = guest_flat_to_host(
        kvm, virtio_guest_to_host_u64(vq->endian, desc[idx].addr));
...
u16 virt_queue__get_iov(struct virt_queue *vq, struct iovec iov[], u16 *out,
                        u16 *in, struct kvm *kvm) {
  u16 head;
  ...
  *out = *in = 0;
  do {
    ...
    if (virt_desc__test_flag(queue, desc, VRING_DESC_F_WRITE)) {
      ...
      out_iov[*out].iov_len =
          virtio_guest_to_host_u32(queue->endian, desc->len);
      (*out)++;
    }
...
bool virtio_queue__should_signal(struct virt_queue *vq) {
  ...
  event_idx =
      virtio_guest_to_host_u16(vq->endian, vring_used_event(&vq->vring));
...
int virtio_init(struct kvm *kvm, void *dev, struct virtio_device *vdev,
                struct virtio_ops *ops, enum virtio_trans trans, int device_id,
...
int virtio_compat_add_message(const char *device, const char *config) {
  ...
  snprintf(desc, len,
           "While you have requested a %s device, "
