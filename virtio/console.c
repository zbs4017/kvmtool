 ...
 */
static void virtio_console__inject_interrupt_callback(struct kvm *kvm,
                                                      void *param) {
...
static int init_vq(struct kvm *kvm, void *dev, u32 vq) {
  ...
  if (vq == VIRTIO_CONSOLE_TX_QUEUE) {
    thread_pool__init_job(&cdev.jobs[vq], kvm, virtio_console_handle_callback,
                          queue);
...
int virtio_console__init(struct kvm *kvm) {
  ...
  if (compat_id == -1)
    compat_id =
        virtio_compat_add_message("virtio-console", "CONFIG_VIRTIO_CONSOLE");
