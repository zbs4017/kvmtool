...
static int virtio_p9_decode(struct p9_pdu *pdu, const char *fmt, va_list ap) {
  ...
  for (ptr = fmt; *ptr; ptr++) {
    ...
    case 'Q': {
      ...
      retval = virtio_p9_pdu_readf(pdu, "bdq", &qid->type, &qid->version,
                                   &qid->path);
    } break;
    case 'S': {
      ...
      retval = virtio_p9_pdu_readf(
          pdu, "wwdQdddqssss", &stbuf->size, &stbuf->type, &stbuf->dev,
          &stbuf->qid, &stbuf->mode, &stbuf->atime, &stbuf->mtime,
          &stbuf->length, &stbuf->name, &stbuf->uid, &stbuf->gid, &stbuf->muid);
      if (retval)
    ...
    case 'I': {
      ...
      retval = virtio_p9_pdu_readf(
          pdu, "ddddqqqqq", &p9attr->valid, &p9attr->mode, &p9attr->uid,
          &p9attr->gid, &p9attr->size, &p9attr->atime_sec, &p9attr->atime_nsec,
...
static int virtio_p9_pdu_encode(struct p9_pdu *pdu, const char *fmt,
                                va_list ap) {
  int retval = 0;
  const char *ptr;
  ...
  for (ptr = fmt; *ptr; ptr++) {
    ...
    case 'Q': {
      struct p9_qid *qid = va_arg(ap, struct p9_qid *);
      retval =
          virtio_p9_pdu_writef(pdu, "bdq", qid->type, qid->version, qid->path);
    } break;
    case 'S': {
      struct p9_wstat *stbuf = va_arg(ap, struct p9_wstat *);
      retval = virtio_p9_pdu_writef(
          pdu, "wwdQdddqssss", stbuf->size, stbuf->type, stbuf->dev,
          &stbuf->qid, stbuf->mode, stbuf->atime, stbuf->mtime, stbuf->length,
          stbuf->name, stbuf->uid, stbuf->gid, stbuf->muid);
    } break;
    case 'A': {
      struct p9_stat_dotl *stbuf = va_arg(ap, struct p9_stat_dotl *);
      retval = virtio_p9_pdu_writef(
          pdu, "qQdddqqqqqqqqqqqqqqq", stbuf->st_result_mask, &stbuf->qid,
          stbuf->st_mode, stbuf->st_uid, stbuf->st_gid, stbuf->st_nlink,
          stbuf->st_rdev, stbuf->st_size, stbuf->st_blksize, stbuf->st_blocks,
          stbuf->st_atime_sec, stbuf->st_atime_nsec, stbuf->st_mtime_sec,
          stbuf->st_mtime_nsec, stbuf->st_ctime_sec, stbuf->st_ctime_nsec,
          stbuf->st_btime_sec, stbuf->st_btime_nsec, stbuf->st_gen,
