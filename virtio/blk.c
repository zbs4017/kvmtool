...
static void virtio_blk_do_io_request(struct kvm *kvm, struct virt_queue *vq,
  ...
  case VIRTIO_BLK_T_GET_ID:
    len =
        disk_image__get_serial(bdev->disk, iov, iovcount, VIRTIO_BLK_ID_BYTES);
    virtio_blk_complete(req, len);
    break;
  default:
...
static void virtio_blk_do_io(struct kvm *kvm, struct virt_queue *vq,
                             struct blk_dev *bdev) {
  struct blk_dev_req *req;
  u16 head;
  ...
  while (virt_queue__available(vq)) {
    ...
    req = &bdev->reqs[head];
    req->head =
        virt_queue__get_head_iov(vq, req->iov, &req->out, &req->in, head, kvm);
    req->vq = vq;
...
static u64 get_host_features(struct kvm *kvm, void *dev) {
  ...
  return 1UL << VIRTIO_BLK_F_SEG_MAX | 1UL << VIRTIO_BLK_F_FLUSH |
         1UL << VIRTIO_RING_F_EVENT_IDX | 1UL << VIRTIO_RING_F_INDIRECT_DESC |
