...
static void virtio_mmio_config_in(struct kvm_cpu *vcpu, u64 addr, u32 *data,
                                  u32 len, struct virtio_device *vdev) {
  ...
  case VIRTIO_MMIO_QUEUE_NUM_MAX:
    val = vdev->ops->get_size_vq(vmmio->kvm, vmmio->dev, vmmio->hdr.queue_sel);
    break;
...
static void virtio_mmio_config_out(struct kvm_cpu *vcpu, u64 addr, u32 *data,
                                   u32 len, struct virtio_device *vdev) {
  struct virtio_mmio *vmmio = vdev->virtio;
  ...
  case VIRTIO_MMIO_DRIVER_FEATURES:
    ...
    virtio_set_guest_features(vmmio->kvm, vdev, vmmio->dev, features);
    break;
  case VIRTIO_MMIO_QUEUE_NUM:
    vmmio->hdr.queue_num = val;
    vdev->ops->set_size_vq(vmmio->kvm, vmmio->dev, vmmio->hdr.queue_sel, val);
    break;
  case VIRTIO_MMIO_QUEUE_READY:
    if (val)
...
void virtio_mmio_modern_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data,
  ...
  if (offset >= VIRTIO_MMIO_CONFIG) {
    ...
    virtio_access_config(vmmio->kvm, vdev, vmmio->dev, offset, data, len,
                         is_write);
    return;
  }

  if (len != 4) {
    pr_debug("Invalid %s size %d at 0x%llx", is_write ? "write" : "read", len,
             addr);
    return;
  }
