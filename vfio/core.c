...
static bool vfio_ioport_in(struct vfio_region *region, u32 offset, void *data,
                           int len) {
...
static bool vfio_ioport_out(struct vfio_region *region, u32 offset, void *data,
                            int len) {
  struct vfio_device *vdev = region->vdev;
  ssize_t nr;
  u32 val;
  ...
  if (nr != len)
    vfio_dev_err(vdev, "could not write %d bytes to I/O port 0x%x", len,
                 offset + region->port_base);
...
err_report:
  vfio_dev_err(vdev, "could not %s %u bytes at 0x%x (0x%llx)",
               is_write ? "write" : "read", len, offset, addr);
}
...
static int vfio_setup_trap_region(struct kvm *kvm, struct vfio_device *vdev,
  ...
  if (region->is_ioport) {
    ...
    port = kvm__register_pio(kvm, region->port_base, region->info.size,
                             vfio_ioport_mmio, region);
    if (port < 0)
      return port;
    return 0;
  }

  return kvm__register_mmio(kvm, region->guest_phys_addr, region->info.size,
                            false, vfio_mmio_access, region);
}
...
int vfio_map_region(struct kvm *kvm, struct vfio_device *vdev,
  ...
  if (base == MAP_FAILED) {
    ...
    vfio_dev_warn(
        vdev,
        "failed to mmap region %u (0x%llx bytes), falling back to trapping",
...
static int vfio_configure_device(struct kvm *kvm, struct vfio_device *vdev) {
  ...
  vdev->fd = ioctl(group->fd, VFIO_GROUP_GET_DEVICE_FD, vdev->params->name);
  if (vdev->fd < 0) {
...
static int vfio_map_mem_bank(struct kvm *kvm, struct kvm_mem_bank *bank,
                             void *data) {
  int ret = 0;
  ...
  if (ioctl(vfio_container, VFIO_IOMMU_MAP_DMA, &dma_map)) {
    ret = -errno;
    pr_err("Failed to map 0x%llx -> 0x%llx (%llu) for DMA", dma_map.iova,
...
static int vfio_unmap_mem_bank(struct kvm *kvm, struct kvm_mem_bank *bank,
                               void *data) {
...
static struct vfio_group *vfio_group_get_for_dev(struct kvm *kvm,
 ...
 * 5. 设置IOMMU类型
 * 6. 映射内存
 *    1. 遍历每个内存区域，调用vfio_map_mem_bank函数进行映射
 *    2.
 * 这个映射，需要映射全部的RAM插槽区域，因为dma操作需要访问全部的物理内存，所以全部都需要映射
 */
