...
#define CPU_ISA_MAX_LEN (ARRAY_SIZE(isa_info_arr) * 16)
    ...
    for (i = 0; i < arr_sz; i++) {
      ...
      pos +=
          snprintf(cpu_isa + pos, CPU_ISA_MAX_LEN, "_%s", isa_info_arr[i].name);
    }
    cpu_isa[pos] = '\0';
    ...
    if (vcpu->riscv_xlen == 64) {
      switch (satp_mode) {
      case 10:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,sv57"));
        break;
      case 9:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,sv48"));
        break;
      case 8:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,sv39"));
        break;
      default:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,none"));
        break;
      }
    } else {
      switch (satp_mode) {
      case 1:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,sv32"));
        break;
      default:
        _FDT(fdt_property_string(fdt, "mmu-type", "riscv,none"));
        break;
    ...
    _FDT(fdt_property_cell(fdt, "phandle", PHANDLE_CPU_INTC_BASE + cpu));
...
static int setup_fdt(struct kvm *kvm) {
  ...
  if (kvm->arch.initrd_size != 0) {
    ...
    u64 ird_end_prop =
        cpu_to_fdt64(kvm->arch.initrd_guest_start + kvm->arch.initrd_size);
  ...
  _FDT(fdt_property_cell(fdt, "#address-cells", 0x2));
  _FDT(fdt_property_cell(fdt, "#size-cells", 0x2));
  _FDT(fdt_property_cell(fdt, "interrupt-parent", riscv_irqchip_phandle));
  ...
  while (dev_hdr) {
    ...
    generate_mmio_fdt_nodes(fdt, dev_hdr, riscv__generate_irq_prop);
