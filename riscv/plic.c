...
static u32 __plic_context_best_pending_irq(struct plic_state *s,
  ...
  for (i = 0; i < s->num_irq_word; i++) {
    ...
    for (j = 0; j < 32; j++) {
      irq = i * 32 + j;
      if ((s->num_irq <= irq) || !(c->irq_pending[i] & (1 << j)) ||
          (c->irq_claimed[i] & (1 << j)))
        continue;

      if (!best_irq || (best_irq_prio < c->irq_pending_priority[irq])) {
        best_irq = irq;
...
static void plic__priority_read(struct plic_state *s, u64 offset, void *data) {
  u32 irq = (offset >> 2);
...
static void plic__priority_write(struct plic_state *s, u64 offset, void *data) {
...
static void plic__context_enable_read(struct plic_state *s,
                                      struct plic_context *c, u64 offset,
                                      void *data) {
...
static void plic__context_enable_write(struct plic_state *s,
  ...
  for (i = 0; i < 32; i++) {
    ...
    if (!(xor_val & irq_mask))
      continue;
    if ((new_val & irq_mask) && (s->irq_level[irq_word] & irq_mask)) {
      c->irq_pending[irq_word] |= irq_mask;
      c->irq_pending_priority[irq] = irq_prio;
...
static void plic__context_read(struct plic_state *s, struct plic_context *c,
...
static void plic__context_write(struct plic_state *s, struct plic_context *c,
                                u64 offset, void *data) {
  ...
  case CONTEXT_CLAIM:
    ...
    irq_word = val / 32;
    irq_mask = 1 << (val % 32);
    if ((val < plic.num_irq) && (c->irq_enable[irq_word] & irq_mask)) {
      c->irq_claimed[irq_word] &= ~irq_mask;
      irq_update = true;
    }
    break;
...
static void plic__mmio_callback(struct kvm_cpu *vcpu, u64 addr, u8 *data,
  ...
  if (is_write) {
    ...
    if (PRIORITY_BASE <= addr && addr < ENABLE_BASE) {
      ...
      addr -= cntx * ENABLE_PER_HART + ENABLE_BASE;
      if (cntx < s->num_context)
        plic__context_enable_read(s, &s->contexts[cntx], addr, data);
    } else if (CONTEXT_BASE <= addr && addr < REG_SIZE) {
      cntx = (addr - CONTEXT_BASE) / CONTEXT_PER_HART;
      addr -= cntx * CONTEXT_PER_HART + CONTEXT_BASE;
      if (cntx < s->num_context)
        plic__context_read(s, &s->contexts[cntx], addr, data);
    }
  }
}
...
static int plic__init(struct kvm *kvm) {
  ...
  ret = kvm__register_mmio(kvm, RISCV_IRQCHIP, RISCV_IRQCHIP_SIZE, false,
                           plic__mmio_callback, &plic);
  if (ret)
    return ret;
