...
struct kvm_cpu *kvm_cpu__arch_init(struct kvm *kvm, unsigned long cpu_id) {
  ...
  for (i = 0; i < KVM_RISCV_SBI_EXT_MAX; i++) {
    if (!kvm->cfg.arch.sbi_ext_disabled[i])
      continue;
    masks[KVM_REG_RISCV_SBI_MULTI_REG(i)] |= KVM_REG_RISCV_SBI_MULTI_MASK(i);
  }
  ...
  if (!kvm->cfg.arch.sbi_ext_disabled[KVM_RISCV_SBI_EXT_DBCN]) {
    id = 1;
    reg.id =
        RISCV_SBI_EXT_REG(KVM_REG_RISCV_SBI_SINGLE, KVM_RISCV_SBI_EXT_DBCN);
    reg.addr = (unsigned long)&id;
    if (ioctl(vcpu->vcpu_fd, KVM_SET_ONE_REG, &reg) < 0)
      pr_warning("KVM_SET_ONE_REG failed (sbi_ext %d)", KVM_RISCV_SBI_EXT_DBCN);
  }
...
static bool kvm_cpu_riscv_sbi(struct kvm_cpu *vcpu) {
  ...
  case SBI_EXT_0_1_CONSOLE_GETCHAR:
    if (term_readable(0))
      vcpu->kvm_run->riscv_sbi.ret[0] = term_getc(vcpu->kvm, 0);
    else
  ...
  case SBI_EXT_DBCN:
    ...
    case SBI_EXT_DBCN_CONSOLE_READ:
      ...
      if (!str_start || !str_end) {
        vcpu->kvm_run->riscv_sbi.ret[0] = SBI_ERR_INVALID_PARAM;
        break;
      }
    ...
    case SBI_EXT_DBCN_CONSOLE_WRITE_BYTE:
      ...
      break;
    default:
      vcpu->kvm_run->riscv_sbi.ret[0] = SBI_ERR_NOT_SUPPORTED;
      break;
    }
    break;
  default:
    ...
    dprintf(dfd, "args[0]=0x%lx args[1]=0x%lx\n",
            vcpu->kvm_run->riscv_sbi.args[0], vcpu->kvm_run->riscv_sbi.args[1]);
    dprintf(dfd, "args[2]=0x%lx args[3]=0x%lx\n",
            vcpu->kvm_run->riscv_sbi.args[2], vcpu->kvm_run->riscv_sbi.args[3]);
    dprintf(dfd, "args[4]=0x%lx args[5]=0x%lx\n",
            vcpu->kvm_run->riscv_sbi.args[4], vcpu->kvm_run->riscv_sbi.args[5]);
    ret = false;
    break;
  };
...
int kvm_cpu__get_endianness(struct kvm_cpu *vcpu) { return VIRTIO_ENDIAN_LE; }
