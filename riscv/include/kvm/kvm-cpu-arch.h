...
static inline __u64 __kvm_reg_id(__u64 type, __u64 subtype, __u64 idx,
                                 __u64 size) {
...
#define RISCV_CONFIG_REG(name)                                                 \
  __kvm_reg_id(KVM_REG_RISCV_CONFIG, 0, KVM_REG_RISCV_CONFIG_REG(name),        \
               KVM_REG_SIZE_ULONG)

#define RISCV_ISA_EXT_REG(id)                                                  \
  __kvm_reg_id(KVM_REG_RISCV_ISA_EXT, 0, id, KVM_REG_SIZE_ULONG)

#define RISCV_CORE_REG(name)                                                   \
  __kvm_reg_id(KVM_REG_RISCV_CORE, 0, KVM_REG_RISCV_CORE_REG(name),            \
               KVM_REG_SIZE_ULONG)

#define RISCV_CSR_REG(name)                                                    \
  __kvm_reg_id(KVM_REG_RISCV_CSR, 0, KVM_REG_RISCV_CSR_REG(name),              \
               KVM_REG_SIZE_ULONG)

#define RISCV_TIMER_REG(name)                                                  \
  __kvm_reg_id(KVM_REG_RISCV_TIMER, 0, KVM_REG_RISCV_TIMER_REG(name),          \
               KVM_REG_SIZE_U64)

#define RISCV_SBI_EXT_REG(subtype, id)                                         \
  __kvm_reg_id(KVM_REG_RISCV_SBI_EXT, subtype, id, KVM_REG_SIZE_ULONG)
...
static inline bool kvm_cpu__emulate_io(struct kvm_cpu *vcpu, u16 port,
                                       void *data, int direction, int size,
                                       u32 count) {
  return false;
}
