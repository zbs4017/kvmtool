...
static inline int qcow_pwrite_sync(int fd, void *buf, size_t count,
                                   off_t offset) {
...
static int qcow_l2_cache_write(struct qcow *q, struct qcow_l2_table *c) {
  ...
  if (qcow_pwrite_sync(q->fd, c->table, size * sizeof(u64), c->offset) < 0)
    return -1;
...
static int qcow_decompress_buffer(u8 *out_buf, int out_buf_size, const u8 *buf,
                                  int buf_size) {
#ifdef CONFIG_HAS_ZLIB
...
static ssize_t qcow1_read_cluster(struct qcow *q, u64 offset, void *dst,
                                  u32 dst_len) {
  struct qcow_header *header = q->header;
  struct qcow_l1_table *l1t = &q->table;
  struct qcow_l2_table *l2t;
  ...
  if (clust_start & QCOW1_OFLAG_COMPRESSED) {
    ...
    if (pread_in_full(q->fd, q->cluster_data, csize, coffset) < 0)
      goto out_error;
...
static ssize_t qcow2_read_cluster(struct qcow *q, u64 offset, void *dst,
                                  u32 dst_len) {
  struct qcow_header *header = q->header;
  struct qcow_l1_table *l1t = &q->table;
  struct qcow_l2_table *l2t;
  ...
  if (clust_start & QCOW2_OFLAG_COMPRESSED) {
    coffset = clust_start & q->cluster_offset_mask;
    nb_csectors = ((clust_start >> q->csize_shift) & q->csize_mask) + 1;
    sector_offset = coffset & (SECTOR_SIZE - 1);
    csize = nb_csectors * SECTOR_SIZE - sector_offset;

    if (pread_in_full(q->fd, q->cluster_data, nb_csectors * SECTOR_SIZE,
                      coffset & ~(SECTOR_SIZE - 1)) < 0) {
      goto out_error;
    }

    if (qcow_decompress_buffer(q->cluster_cache, q->cluster_size,
                               q->cluster_data + sector_offset, csize) < 0) {
      goto out_error;
    }
...
static ssize_t qcow_read_sector_single(struct disk_image *disk, u64 sector,
  ...
  while (nr_read < dst_len) {
    ...
    if (q->version == QCOW1_VERSION)
      ...
      nr = qcow2_read_cluster(q, offset, buf, dst_len - nr_read);
...
static int refcount_block_insert(struct rb_root *root,
                                 struct qcow_refcount_block *new) {
  struct rb_node **link = &(root->rb_node), *parent = NULL;
  u64 offset = new->offset;
...
static int write_refcount_block(struct qcow *q,
                                struct qcow_refcount_block *rfb) {
  if (!rfb->dirty)
    return 0;

  if (qcow_pwrite_sync(q->fd, rfb->entries, rfb->size * sizeof(u16),
                       rfb->offset) < 0)
    return -1;
...
static struct qcow_refcount_block *new_refcount_block(struct qcow *q,
                                                      u64 rfb_offset) {
  struct qcow_refcount_block *rfb;
...
static struct qcow_refcount_block *refcount_block_search(struct qcow *q,
                                                         u64 offset) {
  struct qcow_refcount_table *rft = &q->refcount_table;
  struct qcow_refcount_block *rfb;
...
static struct qcow_refcount_block *qcow_grow_refcount_block(struct qcow *q,
  ...
  if (update_cluster_refcount(q, new_block_offset >> header->cluster_bits, 1) <
      0)
    goto recover_rft;
...
static struct qcow_refcount_block *qcow_read_refcount_block(struct qcow *q,
  ...
  if (pread_in_full(q->fd, rfb->entries, rfb->size * sizeof(u16), rfb_offset) <
      0)
    goto error_free_rfb;
...
static u16 qcow_get_refcount(struct qcow *q, u64 clust_idx) {
  ...
  rfb_idx =
      clust_idx &
      (((1ULL << (header->cluster_bits - QCOW_REFCOUNT_BLOCK_SHIFT)) - 1));
...
again:
  ...
  if (update_ref)
    for (i = 0; i < clust_num; i++)
      if (update_cluster_refcount(q, clust_idx - clust_num + i, 1))
        return -1;
...
static int qcow_write_l1_table(struct qcow *q) {
  ...
  if (qcow_pwrite_sync(q->fd, l1t->l1_table, l1t->table_size * sizeof(u64),
...
static int get_cluster_table(struct qcow *q, u64 offset,
                             struct qcow_l2_table **result_l2t,
                             u64 *result_l2_index) {
  struct qcow_header *header = q->header;
  struct qcow_l1_table *l1t = &q->table;
  struct qcow_l2_table *l2t;
  ...
  if (l2t_offset & QCOW2_OFLAG_COPIED) {
    ...
    l2t_new_offset = qcow_alloc_clusters(q, l2t_size * sizeof(u64), 1);
    ...
    l1t->l1_table[l1t_idx] = cpu_to_be64(l2t_new_offset | QCOW2_OFLAG_COPIED);
 ...
 * modification.
 */
static ssize_t qcow_write_cluster(struct qcow *q, u64 offset, void *buf,
                                  u32 src_len) {
  struct qcow_l2_table *l2t;
  u64 clust_new_start;
  u64 clust_start;
  ...
  if (!(clust_flags & QCOW2_OFLAG_COPIED)) {
    ...
    if (clust_start) {
      mutex_unlock(&q->mutex);
      if (qcow2_read_cluster(q, offset, q->copy_buff, q->cluster_size) < 0) {
    ...
    /* Write actual data */
    if (pwrite_in_full(q->fd, q->copy_buff, q->cluster_size, clust_new_start) <
        0)
      goto free_cluster;

    /* update l2 table*/
    l2t->table[l2t_idx] = cpu_to_be64(clust_new_start | QCOW2_OFLAG_COPIED);
    l2t->dirty = 1;
    ...
    if (clust_flags & QCOW2_OFLAG_COMPRESSED) {
      int size;
      size = ((clust_start >> q->csize_shift) & q->csize_mask) + 1;
      size *= 512;
...
static ssize_t qcow_write_sector_single(struct disk_image *disk, u64 sector,
...
static int qcow_disk_flush(struct disk_image *disk) {
  ...
  list_for_each_safe(pos, n, &rft->lru_list) {
    struct qcow_refcount_block *c =
        list_entry(pos, struct qcow_refcount_block, list);
...
static int qcow_read_refcount_table(struct qcow *q) {
  ...
  rft->rf_size = (header->refcount_table_size * q->cluster_size) / sizeof(u64);
...
static int qcow_write_refcount_table(struct qcow *q) {
  ...
  return qcow_pwrite_sync(q->fd, rft->rf_table, rft->rf_size * sizeof(u64),
                          header->refcount_table_offset);
}
...
static int qcow_read_l1_table(struct qcow *q) {
  ...
  return pread_in_full(q->fd, table->l1_table, sizeof(u64) * table->table_size,
...
static struct disk_image *qcow2_probe(int fd, bool readonly) {
   ...
   */
  if (readonly)
    disk_image = disk_image__new(fd, h->size, &qcow_disk_readonly_ops,
                                 DISK_IMAGE_REGULAR);
  else
    disk_image =
        disk_image__new(fd, h->size, &qcow_disk_ops, DISK_IMAGE_REGULAR);
...
static void *qcow1_read_header(int fd) {
  ...
  *header = (struct qcow_header){
      .size = f_header.size,
      .l1_table_offset = f_header.l1_table_offset,
      .l1_size = f_header.size /
                 ((1 << f_header.l2_bits) * (1 << f_header.cluster_bits)),
      .cluster_bits = f_header.cluster_bits,
