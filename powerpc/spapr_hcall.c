...
static spapr_hcall_fn
    kvmppc_hypercall_table[KVMPPC_HCALL_MAX - KVMPPC_HCALL_BASE + 1];

static target_ulong h_set_dabr(struct kvm_cpu *vcpu, target_ulong opcode,
                               target_ulong *args) {
  /* FIXME:  Implement this for -PR.  (-HV does this in kernel.) */
  return H_HARDWARE;
}

static target_ulong h_rtas(struct kvm_cpu *vcpu, target_ulong opcode,
                           target_ulong *args) {
  ...
  return spapr_rtas_call(vcpu, token, nargs, rtas_r3 + 12, nret,
                         rtas_r3 + 12 + 4 * nargs);
}

static target_ulong h_logical_load(struct kvm_cpu *vcpu, target_ulong opcode,
                                   target_ulong *args) {
...
static target_ulong h_logical_store(struct kvm_cpu *vcpu, target_ulong opcode,
                                    target_ulong *args) {
  /* SLOF will require these, though kernel doesn't. */
  die(__PRETTY_FUNCTION__);
  return H_PARAMETER;
...
static target_ulong h_logical_icbi(struct kvm_cpu *vcpu, target_ulong opcode,
                                   target_ulong *args) {
...
static target_ulong h_logical_dcbf(struct kvm_cpu *vcpu, target_ulong opcode,
                                   target_ulong *args) {
  /* KVM will trap this in the kernel.  Die if it misses. */
  die(__PRETTY_FUNCTION__);
  return H_SUCCESS;
...
static void get_cpu_lpcr(struct kvm_cpu *vcpu, target_ulong *lpcr) {
  struct kvm_one_reg reg = {.id = KVM_REG_PPC_LPCR_64, .addr = (__u64)lpcr};
...
static target_ulong h_set_mode(struct kvm_cpu *vcpu, target_ulong opcode,
                               target_ulong *args) {
  int ret;
...
target_ulong spapr_hypercall(struct kvm_cpu *vcpu, target_ulong opcode,
                             target_ulong *args) {
  if ((opcode <= MAX_HCALL_OPCODE) && ((opcode & 0x3) == 0)) {
    spapr_hcall_fn fn = papr_hypercall_table[opcode / 4];

    if (fn) {
      return fn(vcpu, opcode, args);
    }
  } else if ((opcode >= KVMPPC_HCALL_BASE) && (opcode <= KVMPPC_HCALL_MAX)) {
