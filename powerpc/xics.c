...
static void icp_irq(struct icp_state *icp, int server, int nr,
                    uint8_t priority) {
  struct icp_server_state *ss = icp->ss + server;
  ...
  if ((priority >= CPPR(ss)) ||
    ...
    xics_dprintf("reject %d, CPPR 0x%x, XISR 0x%x, pprio 0x%x, prio 0x%x\n", nr,
                 CPPR(ss), XISR(ss), ss->pending_priority, priority);
...
static void ics_resend(struct ics_state *ics) { ics_resend_msi(ics); }
...
static target_ulong h_cppr(struct kvm_cpu *vcpu, target_ulong opcode,
                           target_ulong *args) {
...
static target_ulong h_ipi(struct kvm_cpu *vcpu, target_ulong opcode,
                          target_ulong *args) {
...
static target_ulong h_xirr(struct kvm_cpu *vcpu, target_ulong opcode,
                           target_ulong *args) {
...
static target_ulong h_eoi(struct kvm_cpu *vcpu, target_ulong opcode,
                          target_ulong *args) {
...
static void rtas_set_xive(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
                          target_ulong args, uint32_t nret, target_ulong rets) {
...
static void rtas_get_xive(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
                          target_ulong args, uint32_t nret, target_ulong rets) {
  struct ics_state *ics = vcpu->kvm->arch.icp->ics;
  uint32_t nr;
...
static void rtas_int_off(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
                         target_ulong args, uint32_t nret, target_ulong rets) {
  struct ics_state *ics = vcpu->kvm->arch.icp->ics;
  uint32_t nr;
...
static void rtas_int_on(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
                        target_ulong args, uint32_t nret, target_ulong rets) {
  struct ics_state *ics = vcpu->kvm->arch.icp->ics;
  uint32_t nr;
