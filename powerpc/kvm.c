...
struct kvm_ext kvm_req_ext[] = {{DEFINE_KVM_EXT(KVM_CAP_PPC_UNSET_IRQ)},
                                {DEFINE_KVM_EXT(KVM_CAP_PPC_IRQ_LEVEL)},
                                {0, 0}};

u64 kvm__arch_default_ram_address(void) { return 0; }
...
bool kvm__arch_cpu_supports_vm(void) { return true; }
...
void kvm__arch_init(struct kvm *kvm) {
  ...
  spapr_hvcons_init();
  spapr_create_phb(kvm, "pci", SPAPR_PCI_BUID, SPAPR_PCI_MEM_WIN_ADDR,
                   SPAPR_PCI_MEM_WIN_SIZE, SPAPR_PCI_IO_WIN_ADDR,
...
bool kvm__arch_load_kernel_image(struct kvm *kvm, int fd_kernel, int fd_initrd,
  ...
  pr_info("Loaded kernel to 0x%x (%ld bytes)", KERNEL_LOAD_ADDR, filesize);
  if (fd_initrd != -1) {
    ...
    filesize = read_file(fd_initrd, p, (kvm->ram_start + kvm->ram_size) - p);
    if (filesize < 0) {
      if (errno == ENOMEM)
    ...
    pr_info("Loaded initrd to 0x%x (%ld bytes)", INITRD_LOAD_ADDR, filesize);
...
static void generate_segment_page_sizes(struct kvm_ppc_smmu_info *info,
...
static int setup_fdt(struct kvm *kvm) {
  ...
  char hypertas_prop_kvm[] =
      "hcall-pft\0hcall-term\0"
  ...
  u32 segment_sizes_1T[] = {cpu_to_be32(0x1c), cpu_to_be32(0x28), 0xffffffff,
                            0xffffffff};
  ...
  if (kvm->arch.initrd_size != 0) {
    ...
    uint32_t ird_end_prop =
        cpu_to_be32(kvm->arch.initrd_gra + kvm->arch.initrd_size);
    _FDT(fdt_property(fdt, "linux,initrd-start", &ird_st_prop,
                      sizeof(ird_st_prop)));
    _FDT(fdt_property(fdt, "linux,initrd-end", &ird_end_prop,
                      sizeof(ird_end_prop)));
  }
   ...
   */
  _FDT(fdt_property_string(fdt, "linux,stdout-path", "/vdevice/vty@30000000"));
  _FDT(fdt_end_node(fdt));
  ...
  for (i = 0; i < smp_cpus; i += SMT_THREADS) {
    ...
    int threads = (smp_cpus - i) >= SMT_THREADS ? SMT_THREADS : smp_cpus - i;
    ...
    _FDT(fdt_property(fdt, "ibm,ppc-interrupt-server#s", servers_prop,
                      threads * sizeof(uint32_t)));
    _FDT(fdt_property(fdt, "ibm,ppc-interrupt-gserver#s", gservers_prop,
                      threads * 2 * sizeof(uint32_t)));

    if (segment_page_sizes.value)
      _FDT(fdt_property(fdt, "ibm,segment-page-sizes", segment_page_sizes.value,
                        segment_page_sizes.size));

    if (cpu_info->mmu_info.flags & KVM_PPC_1T_SEGMENTS)
      _FDT(fdt_property(fdt, "ibm,processor-segment-sizes", segment_sizes_1T,
                        sizeof(segment_sizes_1T)));

    /* VSX / DFP options: */
    if (cpu_info->flags & CPUINFO_FLAG_VMX)
      _FDT(fdt_property_cell(fdt, "ibm,vmx",
                             (cpu_info->flags & CPUINFO_FLAG_VSX) ? 2 : 1));
  ...
  _FDT(fdt_property(fdt, "ibm,interrupt-server-ranges", int_server_ranges_prop,
