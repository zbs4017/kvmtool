...
static void rtas_display_character(struct kvm_cpu *vcpu, uint32_t token,
...
#ifdef RTAS_CONSOLE
static void rtas_put_term_char(struct kvm_cpu *vcpu, uint32_t token,
                               uint32_t nargs, target_ulong args, uint32_t nret,
                               target_ulong rets) {
  char c = rtas_ld(vcpu->kvm, args, 0);
...
static void rtas_get_term_char(struct kvm_cpu *vcpu, uint32_t token,
                               uint32_t nargs, target_ulong args, uint32_t nret,
                               target_ulong rets) {
  int c;
...
static void rtas_get_time_of_day(struct kvm_cpu *vcpu, uint32_t token,
                                 uint32_t nargs, target_ulong args,
                                 uint32_t nret, target_ulong rets) {
  struct tm tm;
  time_t tnow;
...
static void rtas_set_time_of_day(struct kvm_cpu *vcpu, uint32_t token,
                                 uint32_t nargs, target_ulong args,
                                 uint32_t nret, target_ulong rets) {
  pr_warning("%s called; TOD set ignored.\n", __FUNCTION__);
}

static void rtas_power_off(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
...
static void rtas_system_reboot(struct kvm_cpu *vcpu, uint32_t token,
                               uint32_t nargs, target_ulong args, uint32_t nret,
                               target_ulong rets) {
  if (nargs != 0 || nret != 1) {
    rtas_st(vcpu->kvm, rets, 0, -3);
...
static void rtas_query_cpu_stopped_state(struct kvm_cpu *vcpu, uint32_t token,
                                         uint32_t nargs, target_ulong args,
                                         uint32_t nret, target_ulong rets) {
  if (nargs != 1 || nret != 2) {
    rtas_st(vcpu->kvm, rets, 0, -3);
...
static void rtas_start_cpu(struct kvm_cpu *vcpu, uint32_t token, uint32_t nargs,
                           target_ulong args, uint32_t nret,
                           target_ulong rets) {
  die(__FUNCTION__);
}

target_ulong spapr_rtas_call(struct kvm_cpu *vcpu, uint32_t token,
                             uint32_t nargs, target_ulong args, uint32_t nret,
                             target_ulong rets) {
  if ((token >= TOKEN_BASE) && ((token - TOKEN_BASE) < TOKEN_MAX)) {
