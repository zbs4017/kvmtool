...
bool kvm_cpu__emulate_mmio(struct kvm_cpu *vcpu, u64 phys_addr, u8 *data,
  ...
  if ((phys_addr >= SPAPR_PCI_WIN_START) && (phys_addr < SPAPR_PCI_WIN_END)) {
    ret = spapr_phb_mmio(vcpu, phys_addr, data, len, is_write);
  } else {
    pr_warning("MMIO %s unknown address %llx (size %d)!\n",
               is_write ? "write to" : "read from", phys_addr, len);
  }
  return ret;
}
...
void kvm_cpu__show_registers(struct kvm_cpu *vcpu) {
  ...
  dprintf(debug_fd, "\n Registers:\n");
  dprintf(debug_fd,
          " NIP:   %016llx  MSR:   %016llx "
          "( %s%s%s%s%s%s%s%s%s%s%s%s)\n",
          regs.pc, regs.msr, CONDSTR_BIT(regs.msr, SF),
          CONDSTR_BIT(regs.msr, HV), /* ! */
          CONDSTR_BIT(regs.msr, VEC), CONDSTR_BIT(regs.msr, VSX),
          CONDSTR_BIT(regs.msr, EE), CONDSTR_BIT(regs.msr, PR),
          CONDSTR_BIT(regs.msr, FP), CONDSTR_BIT(regs.msr, ME),
          CONDSTR_BIT(regs.msr, IR), CONDSTR_BIT(regs.msr, DR),
          CONDSTR_BIT(regs.msr, RI), CONDSTR_BIT(regs.msr, LE));
...
void kvm_cpu__show_page_tables(struct kvm_cpu *vcpu) { /* Does nothing yet */ }
