...
static void rtas_ibm_read_pci_config(struct kvm_cpu *vcpu, uint32_t token,
                                     uint32_t nargs, target_ulong args,
                                     uint32_t nret, target_ulong rets) {
  uint32_t val = 0;
  uint64_t buid = ((uint64_t)rtas_ld(vcpu->kvm, args, 1) << 32) |
                  rtas_ld(vcpu->kvm, args, 2);
  ...
  phb_dprintf(
      "- cfgRd buid 0x%lx addr 0x%x (/%d): b%d,d%d,f%d,r0x%x, val 0x%x\n", buid,
...
static void rtas_read_pci_config(struct kvm_cpu *vcpu, uint32_t token,
                                 uint32_t nargs, target_ulong args,
                                 uint32_t nret, target_ulong rets) {
  uint32_t val;
  union pci_config_address addr = {.w = rtas_ld(vcpu->kvm, args, 0)};
...
static void rtas_ibm_write_pci_config(struct kvm_cpu *vcpu, uint32_t token,
                                      uint32_t nargs, target_ulong args,
                                      uint32_t nret, target_ulong rets) {
  uint64_t buid = ((uint64_t)rtas_ld(vcpu->kvm, args, 1) << 32) |
                  rtas_ld(vcpu->kvm, args, 2);
  ...
  phb_dprintf(
      "- cfgWr buid 0x%lx addr 0x%x (/%d): b%d,d%d,f%d,r0x%x, val 0x%x\n", buid,
      addr.w, size, addr.bus_number, addr.device_number, addr.function_number,
      addr.register_number, val);
  switch (size) {
  case 4:
...
static void rtas_write_pci_config(struct kvm_cpu *vcpu, uint32_t token,
                                  uint32_t nargs, target_ulong args,
                                  uint32_t nret, target_ulong rets) {
  union pci_config_address addr = {.w = rtas_ld(vcpu->kvm, args, 0)};
  struct pci_device_header *dev = pci__find_dev(addr.device_number);
  ...
  phb_dprintf("- cfgWr addr 0x%x (/%d): b%d,d%d,f%d,r0x%x, val 0x%x\n", addr.w,
              size, addr.bus_number, addr.device_number, addr.function_number,
              addr.register_number, val);
  switch (size) {
  case 4:
    val = le32_to_cpu(val);
...
int spapr_populate_pci_devices(struct kvm *kvm, uint32_t xics_phandle,
                               void *fdt) {
  ...
  struct of_pci_unit64_address {
    ...
    u64 addr;
    u64 size;
  } __attribute((packed)) reg[PCI_NUM_REGIONS + 1],
  ...
  _FDT(fdt_setprop(fdt, bus_off, "interrupt-map-mask", &interrupt_map_mask,
  ...
  while (dev_hdr) {
    ...
    _FDT(fdt_setprop_cell(fdt, node_off, "revision-id", hdr->revision_id));
    _FDT(fdt_setprop_cell(fdt, node_off, "class-code",
                          hdr->class[0] | (hdr->class[1] << 8) |
                              (hdr->class[2] << 16)));
    ...
    reg[0].phys_hi =
        cpu_to_be32(of_pci_b_n(0) | of_pci_b_p(0) | of_pci_b_t(0) |
                    of_pci_b_ss(OF_PCI_SS_CONFIG) | of_pci_b_bbbbbbbb(0) |
                    of_pci_b_ddddd(devid) | of_pci_b_fff(fn));
    reg[0].addr = 0;
    reg[0].size = 0;
    ...
    for (i = 0; i < 6; ++i) {
      ...
      reg[n + 1].phys_hi =
          cpu_to_be32(of_pci_b_n(0) | of_pci_b_p(0) | of_pci_b_t(0) |
                      ...
                      of_pci_b_bbbbbbbb(0) | of_pci_b_ddddd(devid) |
                      of_pci_b_fff(fn) | of_pci_b_rrrrrrrr(bars[i]));
      reg[n + 1].size = cpu_to_be64(pci__bar_size(hdr, i));
      reg[n + 1].addr = 0;

      assigned_addresses[n].phys_hi =
          cpu_to_be32(of_pci_b_n(1) | of_pci_b_p(0) | of_pci_b_t(0) |
                      of_pci_b_ss(bar_to_ss(le32_to_cpu(hdr->bar[i]))) |
                      of_pci_b_bbbbbbbb(0) | of_pci_b_ddddd(devid) |
                      of_pci_b_fff(fn) | of_pci_b_rrrrrrrr(bars[i]));
    ...
    _FDT(fdt_setprop(fdt, node_off, "assigned-addresses", assigned_addresses,
                     sizeof(assigned_addresses[0]) * (n)));
    _FDT(fdt_setprop_cell(fdt, node_off, "interrupts", hdr->irq_pin));
