...
void kvm__init_ram(struct kvm *kvm) {
  ...
  pr_debug("RAM created at 0x%llx - 0x%llx", phys_start,
...
bool kvm__load_firmware(struct kvm *kvm, const char *firmware_filename) {
  ...
  pr_debug("Loaded firmware to 0x%llx (%zd bytes)", kvm->arch.kern_guest_start,
           fw_sz);
  ...
  kvm->arch.dtb_guest_start =
      ALIGN(host_to_guest_flat(kvm, host_pos), FDT_ALIGN);
  pr_debug("Placing fdt at 0x%llx - 0x%llx", kvm->arch.dtb_guest_start,
