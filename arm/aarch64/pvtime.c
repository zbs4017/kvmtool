...
static int pvtime__alloc_region(struct kvm *kvm) {
  ...
  mem = mmap(NULL, ARM_PVTIME_SIZE, PROT_RW, MAP_ANON_NORESERVE, -1, 0);
  if (mem == MAP_FAILED)
    return -errno;
...
int kvm_cpu__setup_pvtime(struct kvm_cpu *vcpu) {
  ...
  u64 pvtime_guest_addr =
      ARM_PVTIME_BASE + vcpu->cpu_id * ARM_PVTIME_STRUCT_SIZE;
  struct kvm_config_arch *kvm_cfg = NULL;
  struct kvm_device_attr pvtime_attr = (struct kvm_device_attr){
      .group = KVM_ARM_VCPU_PVTIME_CTRL, .attr = KVM_ARM_VCPU_PVTIME_IPA};
