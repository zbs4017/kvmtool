...
static struct uip_udp_socket *uip_udp_socket_find(struct uip_tx_arg *arg,
  ...
  list_for_each_entry(sk, sk_head, list) {
    if (sk->sip == sip && sk->dip == dip && sk->sport == sport &&
        sk->dport == dport) {
...
static int uip_udp_socket_send(struct uip_udp_socket *sk, struct uip_udp *udp) {
  ...
  ret = sendto(sk->fd, udp->payload, len, 0, (struct sockaddr *)&sk->addr,
               sizeof(sk->addr));
  if (ret != len)
    return -1;
...
static void *uip_udp_socket_thread(void *p) {
  ...
  while (1) {
    ...
    for (i = 0; i < nfds; i++) {
      ...
      payload_len =
          recvfrom(sk->fd, payload, UIP_MAX_UDP_PAYLOAD, 0, NULL, NULL);
      if (payload_len < 0)
        continue;
...
int uip_tx_do_ipv4_udp(struct uip_tx_arg *arg) {
  ...
  if (!info->udp_thread) {
    ...
    pthread_create(&info->udp_thread, NULL, uip_udp_socket_thread,
                   (void *)info);
  }
