...
static int uip_dhcp_fill_option_name_and_server(struct uip_info *info, u8 *opt,
                                                int i) {
  u8 domain_name_len;
  u32 *addr;
  int n;
...
static int uip_dhcp_fill_option(struct uip_info *info, struct uip_dhcp *dhcp,
                                int reply_msg_type) {
  int i = 0;
  u32 *addr;
  u8 *opt;
...
static int uip_dhcp_make_pkg(struct uip_info *info, struct uip_udp_socket *sk,
                             struct uip_buf *buf, u8 reply_msg_type) {
  struct uip_dhcp *dhcp;
