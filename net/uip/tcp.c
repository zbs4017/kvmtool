...
static struct uip_tcp_socket *uip_tcp_socket_find(struct uip_tx_arg *arg,
...
static struct uip_tcp_socket *uip_tcp_socket_alloc(struct uip_tx_arg *arg,
                                                   u32 sip, u32 dip, u16 sport,
                                                   u16 dport) {
  struct list_head *sk_head;
  struct uip_tcp_socket *sk;
...
static int uip_tcp_payload_send(struct uip_tcp_socket *sk, u8 flag,
                                u16 payload_len) {
...
static int uip_tcp_socket_receive(struct uip_tcp_socket *sk) {
  ...
  if (sk->thread == 0) {
    ...
    if (!sk->buf)
      return -ENOMEM;
    ret = pthread_create(&sk->thread, NULL, uip_tcp_socket_thread, (void *)sk);
    if (ret)
      free(sk->buf);
    return ret;
  }
